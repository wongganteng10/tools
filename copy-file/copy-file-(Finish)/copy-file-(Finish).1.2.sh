...
wrap_text() {
  ...
  local label_width=$((${#label} + 1))
  local max_width=$((width - label_width))

  # Jika panjang value <= max_width → langsung tampilkan
  if [[ ${#value} -le $max_width ]]; then
    echo -e "${label} ${CYAN}${value}${RESET}"
    return
  fi

  # Jika value terlalu panjang → wrap otomatis
  echo -en "${label} "
  echo -e "${CYAN}${value}${RESET}" | fold -s -w "$max_width" | sed "2,\$s/^/$(printf '%*s' $label_width)/"
}
...
tampilkan_info_folder() {
  ...
  wrap_text "Path Lengkap   :" "$folder"
  wrap_text "Ukuran Total   :" "$(du -sh "$folder" | cut -f1)"
  wrap_text "Jumlah File    :" "$(find "$folder" -type f | wc -l)"
  wrap_text "Jumlah Subdir  :" "$(find "$folder" -mindepth 1 -type d | wc -l)"
  wrap_text "Terakhir Ubah  :" "$(stat -c '%y' "$folder" | cut -d'.' -f1)"
